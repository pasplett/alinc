...
class GraphActiveLearningDataModule(L.LightningDataModule):
    """
    ...
    def unlabeled_dataloader(self, subset_size=None):
        ...
        if subset_size is not None:
            unlabeled_indices = self.rng.choice(unlabeled_indices, size=min(
                len(unlabeled_indices), subset_size), replace=False)
    ...
    def labeled_dataloader(self, subset_size=None):
        ...
        if subset_size is not None:
            labeled_indices = self.rng.choice(labeled_indices, size=min(
                len(labeled_indices), subset_size), replace=False)
